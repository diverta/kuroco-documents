...
const getAllComments = async (topics_id) => {
  const { list } = await $fetch(
    `/rcms-api/21/comments?module_id[]=${topics_id}&cnt=999`,
    {
...
const getData = async () => {
  try {
    const apiResponse = await $fetch("/rcms-api/21/newsdetail/1047", {
      baseURL: config.public.apiBase,
      credentials: "include",
    });
    response.value = apiResponse;
    const profileResponse = await $fetch("/rcms-api/21/profile", {
      baseURL: config.public.apiBase,
      credentials: "include",
    });
